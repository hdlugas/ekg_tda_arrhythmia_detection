...
df_ml_input$arr_flag = ifelse(df_ml_input$rhythm == 'SR', 0, 1)
...
for (i in 1:length(param_list_Ks)){
  ...
  for (l in 1:k){
    ...
    trainX = df_model_input[train_idxs, -which(names(df_model_input) %in% c('arr_flag'))]
    testX = df_model_input[test_idxs, -which(names(df_model_input) %in% c('arr_flag'))]
    trainY = df_model_input$arr_flag[train_idxs]
    testY = df_model_input$arr_flag[test_idxs]
...
write.csv(df_output, paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_knn/arr/accuracies_n', as.character(n), '.csv', sep=''), row.names = FALSE)
