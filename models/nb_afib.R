...
for (l in 1:k){
  ...
  mod = naiveBayes(afib_flag ~ ., data=df_model_input[train_idxs,])
  probs = predict(mod, df_model_input[test_idxs,], type='raw')

  path = paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_nb/afib/probs/n', 
                   as.character(n), '_fold', as.character(l), '.csv', sep='')
  write.csv(probs, path, row.names=TRUE)

  true_path = paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_nb/afib/nb_true_fold', as.character(l), '.csv', sep='')
