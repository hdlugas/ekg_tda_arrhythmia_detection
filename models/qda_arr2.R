...
for (l in 1:k){
  ...
  mod = qda(arr_flag ~ ., data=df_model_input[train_idxs,])
  preds_obj = predict(mod, df_model_input[test_idxs,])
  probs = preds_obj[[2]]

  path = paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_qda/arr2/probs/n', 
                   as.character(n), '_fold', as.character(l), '.csv', sep='')
  write.csv(probs, path, row.names=TRUE)

  true_path = paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_qda/arr2/qda_true_fold', as.character(l), '.csv', sep='')
