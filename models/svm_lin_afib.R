...
for (l in 1:k){
  ...
  mod = svm(afib_flag ~ ., data=df_model_input[train_idxs,], kernel='linear')
...
df_output = data.frame(n = ns,
                       accuracy = accuracies, 
                       sensitivity = sensitivities, 
                       specificities = specificities,
                       PPV = PPVs, NPV = NPVs, F1 = F1s)
write.csv(df_output, paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_svm_lin/afib/accuracies_n', as.character(n), '.csv', sep=''), row.names = FALSE)
