...
k_fold_cv_idxs = read.csv('/home/hunter/ekg/afib2/k_fold_cv_arr.csv')
...
for (i in 1:length(param_list_degrees)){
  ...
  for (l in 1:k){
    ...
    mod = svm(arr_flag ~ ., data=df_model_input[train_idxs,], kernel='polynomial', degree=D)
...
write.csv(df_output, paste('/wsu/home/fy/fy73/fy7392/ekg/afib2/ml_output_svm_poly/arr/accuracies_n', as.character(n), '.csv', sep=''), row.names = FALSE)
