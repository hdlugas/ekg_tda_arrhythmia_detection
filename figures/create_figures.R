...
##### get table of the distribution of rhythms #####
...
tmp2$Prop = tmp2$Freq / sum(tmp2$Freq) * 100
tmp2$Prop = sprintf('%0.4f', tmp2$Prop)
...
t = flextable::width(t, j=c(1,2,3), width=c(2.5,2,2))
header = list(Var1 = 'Rhythm', Freq='Count (Total=10605)', Prop='Proportion')
t = set_header_labels(t, values=header)
t = flextable::align(t, j = c(1,2,3), align = 'center', part = 'body')
t = flextable::align(t, align = 'center', part = 'header')
...
get_col_lr = function(path, opt_thresh, opt_N){
  ...
  return(c(paste(sprintf('%0.3f',mean(f1_tmp)), '+-', sprintf('%0.3f',sd(f1_tmp)), sep=' '),
           ...
           paste(sprintf('%0.3f',mean(spec_tmp)), '+-', sprintf('%0.3f',sd(spec_tmp)), sep=' '),
           paste(sprintf('%0.3f',mean(ppv_tmp)), '+-', sprintf('%0.3f',sd(ppv_tmp)), sep=' '),
...
get_col_svm_lin = function(path, opt_N, type){
  ...
  df = read.csv(dir)
  idx = 1
  accs = read.csv(paste('/home/hunter/ekg/afib2/ml_output_svm_lin/', type, '/accuracies_all_folds.csv', sep=''))[,1]
  sens = read.csv(paste('/home/hunter/ekg/afib2/ml_output_svm_lin/', type, '/sensitivities_all_folds.csv', sep=''))[,1]
  specs = read.csv(paste('/home/hunter/ekg/afib2/ml_output_svm_lin/', type, '/specificities_all_folds.csv', sep=''))[,1]
...
V_svm_lin_afib = get_col_svm_lin(afib_dir, opt_N_svm_lin_afib, type='afib')
V_svm_lin_arr = get_col_svm_lin(arr_dir, opt_N_svm_lin_arr, type='arr')
V_svm_lin_arr2 = get_col_svm_lin(arr2_dir, opt_N_svm_lin_arr2, type='arr2')
V_svm_rad_afib = get_col_svm_rad(afib_dir, opt_N_svm_rad_afib, opt_G_svm_rad_afib, type='afib')
V_svm_rad_arr = get_col_svm_rad(arr_dir, opt_N_svm_rad_arr, opt_G_svm_rad_arr, type='arr')
V_svm_rad_arr2 = get_col_svm_rad(arr2_dir, opt_N_svm_rad_arr2, opt_G_svm_rad_arr2, type='arr2')
V_svm_poly_afib = get_col_svm_poly(afib_dir, opt_N_svm_poly_afib, opt_D_svm_poly_afib, type='afib')
...
t = set_header_labels(t, values=header)
t = flextable::align(t, j = seq(1,9,1), align = 'center', part = 'body')
t = flextable::align(t, align = 'center', part = 'header')
...
ggsave(filename = '/home/hunter/ekg/afib2/figures/roc_afib.eps',
       plot = p_afib,
       device = 'eps',
       dpi = 1200,
       width = 12,
...
ggsave(filename = '/home/hunter/ekg/afib2/figures/roc_arr.eps',
       plot = p_arr,
       device = 'eps',
       dpi = 1200,
       width = 12,
...
ggsave(filename = '/home/hunter/ekg/afib2/figures/roc_arr2.eps',
       plot = p_arr2,
       device = 'eps',
       dpi = 1200,
       width = 12,
